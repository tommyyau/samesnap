...
async function handlePut(req: VercelRequest, res: VercelResponse, cardSetId: string) {
...
async function handleDelete(req: VercelRequest, res: VercelResponse, cardSetId: string) {
  const userId = await getUserId(req);
  if (!userId) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Get card set ID from query params
  const cardSetId = req.query.id;
  if (!cardSetId || Array.isArray(cardSetId)) {
    return res.status(400).json({ error: 'Card set ID required' });
  }

  switch (req.method) {
    case 'PUT':
      return handlePut(req, res, cardSetId);
    case 'DELETE':
      return handleDelete(req, res, cardSetId);
    default:
